...
*   **Automated Temperature Threshold (270°C Limit):** The local bridge continuously monitors the temperature data. If the nozzle exceeds the 270°C safety limit, the system enters emergency mode, immediately sending shutdown commands (`M0`/`T0`) to turn off the heater and motor without user intervention.
...
*   **Multi-Tier Safety Safeguards:** Implemented a software-level emergency shutdown protocol that instantly triggers when the heater exceeds the critical safety threshold of 270°C. The system transmits immediate shutdown directives (`M0`/`T0`) to the ESP32, flags the web dashboard with a prominent red emergency overlay, and suspends closed-loop motor adjustments to prevent hardware damage.
